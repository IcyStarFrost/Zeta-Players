...
function ENT:Initialize()
	...
	if ( CLIENT ) then
		...
		self:SetModel(self.useableModels[math.random(#self.useableModels)])
		...
		self.RandomForce = math.random(1,10000)
		local rndmul = 1
		...
		self:SetForce(self.RandomForce,rndmul)

		self:SetEffect( self.useablethrustereffects[math.random(#self.useablethrustereffects)] )

		self:SetOffset( self.ThrustOffset )
		self:StartMotionController()

		self:Switch( tobool(math.random(0,1)) )
		
		self:timer_StartRandom("zetathruster"..self:EntIndex(),1,10,function()
			if !IsValid(self) then timer.Remove("zetathruster"..self:EntIndex()) return end
			
			if GetConVar("zetaplayer_thrusterunfreeze"):GetInt() == 1 then
			local constr = constraint.FindConstraint( self, "Weld" )
			if constr then
				local ent = constr.Ent1 == self and constr.Ent2 or constr.self
				if IsValid(ent) then
		...
		self.SoundName = Sound( self.useableSounds[math.random(#self.useableSounds)] )

	end

end

function ENT:timer_StartRandom(name,min,max,func)
