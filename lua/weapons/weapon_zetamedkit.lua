...
if CLIENT then
language.Add("GMOD_MedKitZeta", "Zeta Compat Medkit")
end

SWEP.PrintName = "#GMOD_MedKitZeta"
SWEP.Author = "robotboy655 & MaxOfS2D. Support for Zeta Players by StarFrost"
...
function SWEP:PrimaryAttack()
	...
	if ( IsValid( ent ) && self:Clip1() >= need && ( ent:IsPlayer() or ent:IsNPC() or ent.IsZetaPlayer ) && ent:Health() < ent:GetMaxHealth() ) then
		...
		if ent.IsZetaPlayer then
			if 100 * math.random() < ent.VoiceChance and CurTime() > ent.HealThanksCooldown   then
				ent.HealThanksCooldown = CurTime()+20
				ent:PlayAssistSound()
			end
		end
